...
def collect_classify_signals(owner: str, repo: str) -> str:
    ...
    Reuses the logic from classify_collector.py without writing to disk.
    """
    # Import inline to avoid circular deps
    import sys, importlib, io
    from collections import Counter

    # Pull constants from classify_collector
    import classify_collector as cc
    ...
    patterns = cc.check_patterns(tree)

    def fmt(s):
        return ", ".join(sorted(s)) if s else "(none)"

    web_ext  = sum(ext_counter.get(e, 0) for e in cc.WEB_APP_EXTENSIONS)
    ml_ext   = sum(ext_counter.get(e, 0) for e in cc.ML_EXTENSIONS)
    infra_ext = sum(ext_counter.get(e, 0) for e in cc.INFRA_EXTENSIONS)
    be_ext   = sum(ext_counter.get(e, 0) for e in cc.BACKEND_EXTENSIONS)
...
Web App:     {fmt(root_files & set(cc.WEB_APP_ROOT_FILE_WEIGHTS))}
Backend:     {fmt(root_files & set(cc.BACKEND_ROOT_FILE_WEIGHTS))}
Library:     {fmt(root_files & cc.GENERIC_MANIFEST_FILES)}
CLI:         {fmt(root_files & set(cc.CLI_ROOT_FILE_WEIGHTS))}
Data/ML:     {fmt(root_files & set(cc.ML_ROOT_FILE_WEIGHTS))}
Infra:       {fmt(root_files & set(cc.INFRA_ROOT_FILE_WEIGHTS))}

--- TOP-LEVEL DIRECTORIES (signal matches) ---
Web App:     {fmt(top_dirs & set(cc.WEB_APP_DIR_WEIGHTS))}
Backend:     {fmt(top_dirs & set(cc.BACKEND_DIR_WEIGHTS))}
Library:     {fmt(top_dirs & set(cc.LIBRARY_DIR_WEIGHTS))}
CLI:         {fmt(top_dirs & set(cc.CLI_DIR_WEIGHTS))}
Data/ML:     {fmt(top_dirs & set(cc.ML_DIR_WEIGHTS))}
Infra:       {fmt(top_dirs & set(cc.INFRA_DIR_WEIGHTS))}
